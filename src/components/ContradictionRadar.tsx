"use client";

import { useEffect, useMemo, useState } from "react";
import {
  ArrowLeft,
  Check,
  CheckCircle2,
  ChevronLeft,
  DatabaseZap,
  FileText,
  Gavel,
  Lightbulb,
  ListChecks,
  Lock,
  Pencil,
  Scale,
  ScrollText,
  Send,
  ShieldAlert,
  Sparkles,
  UserRound,
  type LucideIcon,
} from "lucide-react";
import { useApp } from "@/context/AppContext";
import { useContracts } from "@/context/ContractsContext";
import { useSettings } from "@/context/SettingsContext";
import {
  ASSIGNEES,
  useNotifications,
  type ActivityKind,
} from "@/context/NotificationsContext";
import {
  RESPONSES,
  RESPONSE_TONE_VAR,
  findResponse,
  responseLabel,
  responseNext,
} from "@/lib/actionResponses";
import {
  findContradictions,
  type Contradiction,
  type ContradictionKind,
  type ContradictionSeverity,
} from "@/lib/contradictions";
import type { Contract, Lang } from "@/lib/types";

const KIND_ICON: Record<ContradictionKind, LucideIcon> = {
  residency: DatabaseZap,
  liability: Scale,
  notice: ShieldAlert,
  jurisdiction: Gavel,
};

const SEVERITY_COLOR: Record<ContradictionSeverity, string> = {
  high: "var(--high)",
  medium: "var(--med)",
  low: "var(--low)",
};

// Localised team names (the six assignable teams).
const TEAM_AR: Record<string, string> = {
  "Legal Team": "الفريق القانوني",
  Procurement: "المشتريات",
  Finance: "المالية",
  HR: "الموارد البشرية",
  IT: "تقنية المعلومات",
  Operations: "العمليات",
  "Eman Wed": "إيمان ود",
};
const teamLabel = (name: string, ar: boolean) =>
  ar ? TEAM_AR[name] ?? name : name;

const sevLabel = (sev: ContradictionSeverity, ar: boolean) =>
  ar
    ? sev === "high"
      ? "عالية"
      : sev === "medium"
        ? "متوسطة"
        : "منخفضة"
    : sev;

function timeAgo(at: number, lang: Lang): string {
  const s = Math.floor((Date.now() - at) / 1000);
  if (s < 60) return lang === "ar" ? "الآن" : "just now";
  const m = Math.floor(s / 60);
  if (m < 60) return lang === "ar" ? `قبل ${m} د` : `${m}m ago`;
  const h = Math.floor(m / 60);
  if (h < 24) return lang === "ar" ? `قبل ${h} س` : `${h}h ago`;
  const d = Math.floor(h / 24);
  return lang === "ar" ? `قبل ${d} ي` : `${d}d ago`;
}

/**
 * Cross-portfolio "contradiction radar": each finding is a compact card that
 * opens a detail panel — related contracts, assignee, resolution outcomes,
 * comments and an activity log — mirroring the Action Center's alert detail.
 * Action state lives in the shared notifications store under `contra-<id>`
 * keys, so it drives the sidebar counter and clears on demo reset.
 */
export function ContradictionRadar({
  onOpen,
}: {
  onOpen: (c: Contract) => void;
}) {
  const { L, lang } = useApp();
  const { contracts } = useContracts();
  const { checks } = useSettings();
  const { getState } = useNotifications();
  const ar = lang === "ar";

  const [selected, setSelected] = useState<Contradiction | null>(null);

  const items = useMemo(
    () => findContradictions(contracts, checks),
    [contracts, checks],
  );

  const byId = useMemo(() => {
    const m = new Map<string, Contract>();
    for (const c of contracts) m.set(c.id, c);
    return m;
  }, [contracts]);

  const openItems = items.filter((it) => !getState(`contra-${it.id}`).done);
  const resolvedCount = items.length - openItems.length;
  const highCount = openItems.filter((i) => i.severity === "high").length;

  return (
    <>
      <div className="flex flex-col gap-4">
        {/* Intro / summary — plain description under the page title (no box). */}
        <div>
          <p style={{ fontSize: 13.5, color: "var(--text-soft)", lineHeight: 1.7 }}>
            {L.radarIntro}
          </p>
          {items.length > 0 && (
            <div style={{ fontSize: 12.5, color: "var(--text-soft)", marginTop: 8 }}>
              <b style={{ color: "var(--text)" }}>
                {ar ? `${openItems.length} تعارض مفتوح` : `${openItems.length} open conflicts`}
              </b>
              {highCount > 0 &&
                (ar ? ` · ${highCount} بأولوية عالية` : ` · ${highCount} high priority`)}
              {resolvedCount > 0 && ` · ${resolvedCount} ${L.radarResolvedCount}`}
            </div>
          )}
        </div>

        {items.length === 0 ? (
          <div
            className="flex flex-col items-center text-center gap-2"
            style={{
              background: "var(--surface)",
              border: "1px solid var(--border)",
              borderRadius: 14,
              padding: "40px 20px",
            }}
          >
            <CheckCircle2 size={30} color="var(--low)" />
            <div style={{ fontSize: 14.5, fontWeight: 700, color: "var(--text)" }}>
              {L.radarEmptyTitle}
            </div>
            <div style={{ fontSize: 13, color: "var(--text-soft)", maxWidth: 380 }}>
              {L.radarEmpty}
            </div>
          </div>
        ) : (
          items.map((it) => (
            <ContradictionCard key={it.id} item={it} onSelect={setSelected} ar={ar} />
          ))
        )}
      </div>

      {selected && (
        <ContradictionDetail
          item={selected}
          byId={byId}
          onClose={() => setSelected(null)}
          onOpenContract={(c) => {
            setSelected(null);
            onOpen(c);
          }}
        />
      )}
    </>
  );
}

/** Compact list card — summary + status, opens the detail panel on click. */
function ContradictionCard({
  item,
  onSelect,
  ar,
}: {
  item: Contradiction;
  onSelect: (it: Contradiction) => void;
  ar: boolean;
}) {
  const { L, lang } = useApp();
  const { getState } = useNotifications();
  const st = getState(`contra-${item.id}`);
  const done = st.done;
  const outcome = findResponse("anomaly", st.outcome?.key ?? undefined);
  const Icon = KIND_ICON[item.kind];
  const color = SEVERITY_COLOR[item.severity];

  return (
    <button
      onClick={() => onSelect(item)}
      className="tap text-start w-full flex items-start gap-3 p-3.5"
      style={{
        background: "var(--surface)",
        border: "1px solid var(--border)",
        borderRadius: 14,
        boxShadow: "var(--shadow)",
        opacity: done ? 0.72 : 1,
      }}
    >
      <span
        className="flex items-center justify-center shrink-0"
        style={{
          width: 36,
          height: 36,
          borderRadius: 10,
          background: `color-mix(in srgb, ${done ? "var(--text-soft)" : color} 15%, transparent)`,
        }}
      >
        <Icon size={17} color={done ? "var(--text-soft)" : color} />
      </span>
      <div className="min-w-0 flex-1">
        <div className="flex items-center justify-between gap-2 flex-wrap">
          <span
            style={{
              fontSize: 14,
              fontWeight: 700,
              color: done ? "var(--text-soft)" : "var(--text)",
              textDecoration: done ? "line-through" : "none",
            }}
          >
            {ar ? item.title.ar : item.title.en}
          </span>
          {done && outcome ? (
            <span
              className="inline-flex items-center gap-1 whitespace-nowrap"
              style={{
                fontSize: 11,
                fontWeight: 700,
                color: RESPONSE_TONE_VAR[outcome.tone],
                background: `color-mix(in srgb, ${RESPONSE_TONE_VAR[outcome.tone]} 15%, transparent)`,
                borderRadius: 999,
                padding: "2px 9px",
              }}
            >
              <Check size={11} /> {responseLabel(outcome, lang)}
            </span>
          ) : (
            <span
              className="whitespace-nowrap"
              style={{
                fontSize: 11,
                fontWeight: 700,
                color,
                background: `color-mix(in srgb, ${color} 12%, transparent)`,
                borderRadius: 20,
                padding: "2px 9px",
                textTransform: "capitalize",
              }}
            >
              {sevLabel(item.severity, ar)}
            </span>
          )}
        </div>
        <p
          style={{
            fontSize: 12.5,
            color: "var(--text-soft)",
            lineHeight: 1.55,
            marginTop: 4,
            display: "-webkit-box",
            WebkitLineClamp: 2,
            WebkitBoxOrient: "vertical",
            overflow: "hidden",
          }}
        >
          {ar ? item.detail.ar : item.detail.en}
        </p>
        <div
          className="flex items-center gap-x-3 gap-y-1 flex-wrap mt-2"
          style={{ fontSize: 12, color: "var(--text-soft)" }}
        >
          <span className="inline-flex items-center gap-1">
            <FileText size={12} /> {item.contractIds.length} {L.radarConflictCards}
          </span>
        </div>
        {st.assignee && (
          <div className="mt-2">
            <span
              className="inline-flex items-center gap-1"
              style={{
                fontSize: 11.5,
                fontWeight: 600,
                color: "var(--accent)",
                background: "var(--accent-soft)",
                borderRadius: 999,
                padding: "2px 9px",
              }}
            >
              <UserRound size={11} /> {teamLabel(st.assignee, ar)}
            </span>
          </div>
        )}
      </div>
    </button>
  );
}

/**
 * Contradiction detail side panel — mirrors the Action Center's AlertDetail:
 * related contracts, assignee (Pro), resolution outcomes (anomaly group),
 * comments and activity log. Everything keyed by `contra-<id>`.
 */
function ContradictionDetail({
  item,
  byId,
  onClose,
  onOpenContract,
}: {
  item: Contradiction;
  byId: Map<string, Contract>;
  onClose: () => void;
  onOpenContract: (c: Contract) => void;
}) {
  const { lang, L, dir, plan, setUpgradeOpen } = useApp();
  const ar = lang === "ar";
  const isPro = plan === "paid";
  const { getState, toggleDone, resolveWithOutcome, setAssignee, addComment } =
    useNotifications();

  const key = `contra-${item.id}`;
  const st = getState(key);
  const [comment, setComment] = useState("");

  const committedAssignee = st.assignee ?? "";
  const [pendingAssignee, setPendingAssignee] = useState(committedAssignee);
  const [editingAssignee, setEditingAssignee] = useState(false);

  useEffect(() => {
    const onKey = (e: KeyboardEvent) => e.key === "Escape" && onClose();
    document.addEventListener("keydown", onKey);
    const prev = document.body.style.overflow;
    document.body.style.overflow = "hidden";
    return () => {
      document.removeEventListener("keydown", onKey);
      document.body.style.overflow = prev;
    };
  }, [onClose]);

  const Icon = KIND_ICON[item.kind];
  const color = SEVERITY_COLOR[item.severity];
  const responses = RESPONSES.anomaly;
  const chosen = findResponse("anomaly", st.outcome?.key ?? undefined);

  const submitComment = () => {
    const t = comment.trim();
    if (t) {
      addComment(key, t);
      setComment("");
    }
  };

  const actText = (kind: ActivityKind): string =>
    ({
      done: L.actDone,
      reopen: L.actReopen,
      assigned: L.actAssigned,
      unassigned: L.actUnassigned,
      tag_add: L.actTagAdd,
      tag_remove: L.actTagRemove,
      comment: L.actComment,
      outcome: L.actOutcome,
    })[kind];

  const activityValue = (a: { kind: ActivityKind; value?: string }): string => {
    if (a.kind === "outcome" && a.value) {
      const [group, k] = a.value.split(":");
      const r = findResponse(group as never, k);
      return r ? responseLabel(r, lang) : a.value;
    }
    return a.value ? teamLabel(a.value, ar) : "";
  };

  return (
    <div
      className="fixed inset-0 z-[65] flex"
      style={{ justifyContent: dir === "rtl" ? "flex-start" : "flex-end" }}
      role="dialog"
      aria-modal="true"
    >
      <div
        onClick={onClose}
        style={{
          position: "absolute",
          inset: 0,
          background: "rgba(0,0,0,.45)",
          backdropFilter: "blur(2px)",
        }}
      />
      <div
        className="rise scroll-slim"
        style={{
          position: "relative",
          width: "min(480px,100%)",
          height: "100%",
          background: "var(--bg)",
          borderInlineStart: "1px solid var(--border)",
          overflowY: "auto",
          boxShadow: "var(--shadow)",
          display: "flex",
          flexDirection: "column",
        }}
      >
        {/* Header */}
        <div
          className="flex items-center justify-between gap-2 px-5 py-4 sticky top-0 z-[2]"
          style={{
            background: "color-mix(in srgb, var(--bg) 95%, transparent)",
            backdropFilter: "blur(8px)",
            borderBottom: "1px solid var(--border)",
          }}
        >
          <button
            onClick={onClose}
            className="tap flex items-center gap-1"
            style={{ fontSize: 13, color: "var(--text-soft)", background: "none", border: "none", cursor: "pointer" }}
          >
            {dir === "rtl" ? (
              <ChevronLeft size={16} style={{ transform: "scaleX(-1)" }} />
            ) : (
              <ArrowLeft size={16} />
            )}{" "}
            {L.back}
          </button>
          {st.done && (
            <span
              className="inline-flex items-center gap-1.5"
              style={{
                fontSize: 12.5,
                fontWeight: 700,
                color: "var(--high)",
                background: "var(--high-bg)",
                border: "1px solid var(--high)",
                borderRadius: 9,
                padding: "6px 12px",
              }}
            >
              <Check size={14} /> {L.taskDone}
            </span>
          )}
        </div>

        <div className="px-5 py-5" style={{ flex: 1 }}>
          {/* Summary */}
          <div className="flex items-start gap-2.5">
            <span
              className="flex items-center justify-center shrink-0"
              style={{
                width: 34,
                height: 34,
                borderRadius: 9,
                background: `color-mix(in srgb, ${color} 16%, transparent)`,
              }}
            >
              <Icon size={17} color={color} />
            </span>
            <div className="min-w-0 flex-1">
              <div className="flex items-center gap-2 flex-wrap">
                <h2
                  style={{
                    fontSize: 16,
                    fontWeight: 700,
                    lineHeight: 1.4,
                    textDecoration: st.done ? "line-through" : "none",
                    color: st.done ? "var(--text-soft)" : "var(--text)",
                  }}
                >
                  {ar ? item.title.ar : item.title.en}
                </h2>
                <span
                  style={{
                    fontSize: 11,
                    fontWeight: 700,
                    color,
                    background: `color-mix(in srgb, ${color} 12%, transparent)`,
                    borderRadius: 20,
                    padding: "2px 9px",
                    textTransform: "capitalize",
                  }}
                >
                  {sevLabel(item.severity, ar)}
                </span>
              </div>
              <p style={{ fontSize: 13, color: "var(--text-soft)", lineHeight: 1.65, marginTop: 6 }}>
                {ar ? item.detail.ar : item.detail.en}
              </p>
            </div>
          </div>

          {/* Recommendation */}
          <div
            className="flex items-start gap-2 mt-4 p-3"
            style={{
              background: "var(--accent-soft)",
              border: "1px solid color-mix(in srgb, var(--accent) 25%, var(--border))",
              borderRadius: 12,
            }}
          >
            <Lightbulb size={15} color="var(--accent)" className="shrink-0" style={{ marginTop: 1 }} />
            <div>
              <div
                style={{
                  fontSize: 10.5,
                  fontWeight: 800,
                  color: "var(--accent)",
                  letterSpacing: 0.3,
                  textTransform: "uppercase",
                }}
              >
                {L.radarRec}
              </div>
              <p style={{ fontSize: 12.5, color: "var(--text)", lineHeight: 1.6, marginTop: 3 }}>
                {ar ? item.recommendation.ar : item.recommendation.en}
              </p>
            </div>
          </div>

          {/* Related contracts — open any in the drawer */}
          <Section title={L.radarRelated} icon={FileText}>
            <div className="flex flex-col gap-2">
              {item.contractIds.map((id) => {
                const c = byId.get(id);
                if (!c) return null;
                return (
                  <button
                    key={id}
                    onClick={() => onOpenContract(c)}
                    className="tap flex items-center gap-2 text-start"
                    style={{
                      background: "var(--surface)",
                      border: "1px solid var(--border)",
                      borderRadius: 10,
                      padding: "9px 11px",
                    }}
                  >
                    <FileText size={14} color="var(--accent)" className="shrink-0" />
                    <span className="min-w-0 flex-1 truncate" style={{ fontSize: 12.5, fontWeight: 600, color: "var(--text)" }}>
                      {ar ? c.title_ar : c.title_en}
                    </span>
                    <span style={{ fontSize: 11, color: "var(--text-soft)" }}>{c.id}</span>
                  </button>
                );
              })}
            </div>
          </Section>

          {/* Assignee — Pro; reassigning is confirmed */}
          <Section title={L.taskAssignee} icon={UserRound}>
            {!isPro ? (
              <button
                onClick={() => setUpgradeOpen(true)}
                className="tap w-full flex items-start gap-2 p-3 text-start"
                style={{
                  background: "var(--accent-soft)",
                  border: "1px dashed var(--accent)",
                  borderRadius: 12,
                  color: "var(--accent)",
                }}
              >
                <Lock size={15} className="shrink-0" style={{ marginTop: 1 }} />
                <span style={{ fontSize: 12.5, fontWeight: 600, lineHeight: 1.5 }}>
                  {ar
                    ? "إسناد الإجراءات إلى الفرق متاح في الخطة الاحترافية."
                    : "Assigning actions to teams is a Pro feature."}
                </span>
              </button>
            ) : !editingAssignee ? (
              <div className="flex items-center gap-2 flex-wrap">
                <span
                  className="inline-flex items-center gap-1.5"
                  style={{
                    fontSize: 12.5,
                    fontWeight: 600,
                    color: "var(--accent)",
                    background: "var(--accent-soft)",
                    borderRadius: 999,
                    padding: "5px 12px",
                  }}
                >
                  <UserRound size={13} />
                  {committedAssignee ? teamLabel(committedAssignee, ar) : L.taskUnassigned}
                </span>
                <button
                  onClick={() => setEditingAssignee(true)}
                  className="tap inline-flex items-center gap-1"
                  style={{ fontSize: 12, fontWeight: 600, color: "var(--text-soft)", background: "none", border: "none", cursor: "pointer" }}
                >
                  <Pencil size={12} /> {ar ? "تغيير" : "Change"}
                </button>
              </div>
            ) : (
              <>
                <select
                  value={pendingAssignee}
                  onChange={(e) => setPendingAssignee(e.target.value)}
                  className="w-full"
                  style={{
                    background: "var(--surface)",
                    border: `1px solid ${pendingAssignee !== committedAssignee ? "var(--accent)" : "var(--border)"}`,
                    borderRadius: 9,
                    padding: "9px 10px",
                    fontSize: 13,
                    color: "var(--text)",
                    fontFamily: "inherit",
                  }}
                >
                  <option value="">{L.taskUnassigned}</option>
                  {ASSIGNEES.map((a) => (
                    <option key={a} value={a}>
                      {teamLabel(a, ar)}
                    </option>
                  ))}
                </select>
                <div className="flex items-center gap-2 mt-2 flex-wrap">
                  <button
                    onClick={() => {
                      setAssignee(key, pendingAssignee || null);
                      setEditingAssignee(false);
                    }}
                    disabled={pendingAssignee === committedAssignee}
                    className="tap inline-flex items-center gap-1.5"
                    style={{
                      fontSize: 12.5,
                      fontWeight: 700,
                      color: "var(--on-accent)",
                      background: "var(--grad-primary)",
                      border: "none",
                      borderRadius: 9,
                      padding: "6px 14px",
                      cursor: "pointer",
                      opacity: pendingAssignee === committedAssignee ? 0.5 : 1,
                    }}
                  >
                    <Check size={13} /> {ar ? "تأكيد" : "Confirm"}
                  </button>
                  <button
                    onClick={() => {
                      setPendingAssignee(committedAssignee);
                      setEditingAssignee(false);
                    }}
                    className="tap"
                    style={{
                      fontSize: 12.5,
                      fontWeight: 600,
                      color: "var(--text-soft)",
                      background: "var(--surface)",
                      border: "1px solid var(--border)",
                      borderRadius: 9,
                      padding: "6px 14px",
                      cursor: "pointer",
                    }}
                  >
                    {ar ? "إلغاء" : "Cancel"}
                  </button>
                </div>
              </>
            )}
          </Section>

          {/* Action taken — log the resolution outcome */}
          <Section title={L.taskOutcome} icon={ListChecks}>
            <div style={{ fontSize: 12, color: "var(--text-soft)", lineHeight: 1.5, marginBottom: 10 }}>
              {L.taskOutcomePrompt}
            </div>
            <div className="flex flex-col gap-2">
              {responses.map((r) => {
                const on = st.outcome?.key === r.key;
                const tone = RESPONSE_TONE_VAR[r.tone];
                return (
                  <button
                    key={r.key}
                    onClick={() =>
                      on ? toggleDone(key) : resolveWithOutcome(key, "anomaly", r.key)
                    }
                    aria-pressed={on}
                    className="tap flex items-center gap-3 text-start"
                    style={{
                      borderRadius: 12,
                      padding: "11px 13px",
                      fontSize: 13.5,
                      fontWeight: on ? 600 : 500,
                      color: "var(--text)",
                      background: on
                        ? `color-mix(in srgb, ${tone} 7%, var(--surface))`
                        : "var(--surface)",
                      border: `1px solid ${on ? `color-mix(in srgb, ${tone} 45%, var(--border))` : "var(--border)"}`,
                      boxShadow: "var(--shadow)",
                      transition: "background .15s, border-color .15s",
                    }}
                  >
                    <span
                      aria-hidden
                      className="flex items-center justify-center shrink-0"
                      style={{
                        width: 19,
                        height: 19,
                        borderRadius: 999,
                        background: on ? tone : "transparent",
                        border: `1.5px solid ${on ? tone : "var(--border)"}`,
                        color: "var(--on-accent)",
                        transition: "background .15s, border-color .15s",
                      }}
                    >
                      {on && <Check size={12} strokeWidth={3} />}
                    </span>
                    <span className="flex-1">{responseLabel(r, lang)}</span>
                  </button>
                );
              })}
            </div>

            {chosen && (
              <div
                className="rise flex items-start gap-3 mt-4 p-4"
                style={{
                  background:
                    "linear-gradient(155deg, var(--accent-soft) 0%, color-mix(in srgb, var(--accent) 9%, var(--surface)) 100%)",
                  border: "1px solid color-mix(in srgb, var(--accent) 30%, var(--border))",
                  borderRadius: 16,
                  boxShadow: "var(--shadow)",
                }}
              >
                <span
                  className="flex items-center justify-center shrink-0"
                  style={{
                    width: 32,
                    height: 32,
                    borderRadius: 10,
                    background: "var(--accent)",
                    color: "var(--on-accent)",
                    boxShadow: "0 2px 6px color-mix(in srgb, var(--accent) 45%, transparent)",
                  }}
                >
                  <Sparkles size={16} />
                </span>
                <div className="min-w-0 flex-1">
                  <div
                    className="inline-flex items-center gap-1.5"
                    style={{
                      fontSize: 11,
                      fontWeight: 800,
                      color: "var(--accent)",
                      letterSpacing: 0.4,
                      textTransform: "uppercase",
                    }}
                  >
                    {L.taskNextStep}
                  </div>
                  <div style={{ fontSize: 13, color: "var(--text)", lineHeight: 1.7, marginTop: 4 }}>
                    {responseNext(chosen, lang)}
                  </div>
                </div>
              </div>
            )}
          </Section>

          {/* Comments */}
          <Section title={L.taskComments} icon={FileText}>
            <div className="flex flex-col gap-2 mb-2">
              {st.comments.length === 0 && (
                <div style={{ fontSize: 12.5, color: "var(--text-soft)" }}>
                  {L.taskNoComments}
                </div>
              )}
              {st.comments.map((cm) => (
                <div
                  key={cm.id}
                  style={{
                    background: "var(--surface)",
                    border: "1px solid var(--border)",
                    borderRadius: 10,
                    padding: "8px 10px",
                  }}
                >
                  <div className="flex items-center justify-between gap-2 mb-0.5">
                    <span style={{ fontSize: 12, fontWeight: 700 }}>
                      {teamLabel(cm.author, ar)}
                    </span>
                    <span style={{ fontSize: 10.5, color: "var(--text-soft)" }}>
                      {timeAgo(cm.at, lang)}
                    </span>
                  </div>
                  <div style={{ fontSize: 12.5, color: "var(--text)", lineHeight: 1.6 }}>
                    {cm.text}
                  </div>
                </div>
              ))}
            </div>
            <div className="flex items-end gap-2">
              <textarea
                value={comment}
                onChange={(e) => setComment(e.target.value)}
                onKeyDown={(e) => {
                  if (e.key === "Enter" && !e.shiftKey && !e.altKey) {
                    e.preventDefault();
                    submitComment();
                  }
                }}
                placeholder={L.taskAddComment}
                rows={2}
                className="flex-1"
                style={{
                  background: "var(--surface)",
                  border: "1px solid var(--border)",
                  borderRadius: 9,
                  padding: "8px 10px",
                  fontSize: 12.5,
                  color: "var(--text)",
                  fontFamily: "inherit",
                  resize: "vertical",
                }}
              />
              <button
                onClick={submitComment}
                disabled={!comment.trim()}
                className="tap flex items-center justify-center shrink-0"
                style={{
                  width: 38,
                  height: 38,
                  borderRadius: 9,
                  background: "var(--grad-primary)",
                  color: "var(--on-accent)",
                  border: "none",
                  opacity: comment.trim() ? 1 : 0.5,
                }}
                aria-label={L.taskSend}
              >
                <Send size={16} className="rtl-flip" />
              </button>
            </div>
          </Section>

          {/* Activity log */}
          <Section title={L.taskActivity} icon={ScrollText}>
            {st.activity.length === 0 ? (
              <div style={{ fontSize: 12.5, color: "var(--text-soft)" }}>
                {L.taskNoActivity}
              </div>
            ) : (
              <div className="flex flex-col gap-2.5">
                {st.activity.map((a) => (
                  <div key={a.id} className="flex items-start gap-2">
                    <span
                      aria-hidden
                      className="shrink-0"
                      style={{ width: 7, height: 7, borderRadius: 7, background: "var(--accent)", marginTop: 5 }}
                    />
                    <div className="min-w-0 flex-1">
                      <div style={{ fontSize: 12.5, color: "var(--text)", lineHeight: 1.5 }}>
                        <b style={{ fontWeight: 700 }}>{teamLabel(a.author, ar)}</b>{" "}
                        {actText(a.kind)}
                        {activityValue(a) ? ` ${activityValue(a)}` : ""}
                      </div>
                      <div style={{ fontSize: 10.5, color: "var(--text-soft)", marginTop: 1 }}>
                        {timeAgo(a.at, lang)}
                      </div>
                    </div>
                  </div>
                ))}
              </div>
            )}
          </Section>
        </div>
      </div>
    </div>
  );
}

function Section({
  title,
  icon: Icon,
  children,
}: {
  title: string;
  icon: LucideIcon;
  children: React.ReactNode;
}) {
  return (
    <div className="mt-5">
      <div
        className="flex items-center gap-1.5 mb-2"
        style={{ fontSize: 12, fontWeight: 700, color: "var(--text-soft)" }}
      >
        <Icon size={14} /> {title}
      </div>
      {children}
    </div>
  );
}
